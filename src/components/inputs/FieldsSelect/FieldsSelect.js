...
const FieldsSelect = ({ initialFields, updateFields, children }) => {
  ...
  useEffect(() => {
    const initialMajorFieldValue = getMajorFieldsFromSubfields(majorFieldsOptions, initialFields);
    setSelectedMajorFields(initialMajorFieldValue);
  }, [majorFieldsOptions, initialFields]);
  ...
  const availableSubFieldsOptions = useMemo(() => {
    ...
    if (!!selectedMajorFields && selectedMajorFields.length > 0) {
      const selectedMajorFieldsIDs = selectedMajorFields.map((majorField) => majorField.id);
      ...
      const availableSubFields = availableMajorFields?.map((majorField) => (getSubFieldsOptions(majorField))).flat();
  ...
  const MajorField = () => (<AutoCompleteSelectField
  ...
  const Field = () => (<AutoCompleteSelectField
